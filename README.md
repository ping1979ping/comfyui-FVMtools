...
- **JB (JSON Builder)** — universal JSON-prompt assembly suite. Five nodes: `JB · Outfit Block` and `JB · Location Block` (one-shot combos with built-in palette + Edit List), `JB · Builder` (hand-author a tree of `key:value` rows with indent-based nesting + Insert From Catalog dropdown + Edit Catalog modal), `JB · Stitcher` (deep-merge N JSON fragments under one title, dynamic input slots), and `JB · Extractor` (pull a sub-tree by dot-path). Output is plain string (strict JSON or loose-keys, no-key-quotes form) ready for any CLIPTextEncode. See [JB reference workflow](examples/workflows/jb_v1_minimal.md).
  - For **Krea 2 / Qwen3-VL text encoders**, set `output_format: natural` — plain prose with no keys, braces or metadata, since those encoders read the JSON instead of obeying it — or `sentences`, which keeps what the keys mean as one deterministic sentence per fact (*The scene takes place indoors, it is a family event, namely the graduation party at home. The background is …*). A single `color_mood` dropdown replaces the five harmony sliders, and 16 **everyday** outfit/location sets provide plain amateur-style material instead of studio looks. See [JB for Krea 2](documentation/JB_NATURAL_PROMPTS.md).
...
- **K2 Lab (Krea 2 regional control)** — 22 nodes for pixel-space region prompting on Krea 2, including a **visual region builder** with a detached editor window (draw boxes on a canvas over a recent render, one prompt and any number of LoRAs per box, aspect-ratio changes rescale the layout shape-preserving): a spatial attention router that binds each region's text tokens to the image tokens inside its box, strict regional LoRA routing (two characters, two LoRAs, **one** denoising pass — no crop compositing, works on FP8/INT8 checkpoints), `txtfusion.projector` control incl. exact extraction from projector LoRAs, per-region face refinement, regional editing and K2Lab project import/export. See [K2 Lab guide](documentation/K2_LAB.md).
- **Sign Tools (text repair)** — four nodes that find, judge and re-render unbelievable lettering. `Sign Selector SAM3` grounds nine text classes (sign, label, garment print, poster, screen, book, license plate, paper, graffiti) against a **single** SAM3 vision encode, gates them by minimum readable height, scores how implausible the existing lettering is, and clusters near-identical regions so a shelf of twelve identical bottles becomes one decision. `Sign Text Proposer` asks a vision model in LM Studio — over plain HTTP, no other extension needed — what each sign should actually say, seeing the crop *and* the whole scene so the answer fits the setting. `Sign Detailer` re-renders with **glyph guidance**: the target text is typeset, warped onto the sign's real quadrilateral and composited into the crop before encoding, so the sampler only restyles material and lighting instead of inventing letterforms — the AnyText/GlyphControl idea without an extra model, which is why it also works with text-weak checkpoints. Regions too small to ever read are softened into believable out-of-focus text rather than forced. See [Sign Guide](documentation/SIGN_GUIDE.md).
  - Optional OCR backend (~16 MB ONNX, or EasyOCR) sharpens the slop detection — the strongest signal is the contradiction *SAM3 sees text, OCR reads nothing*. Without it the judgement falls back to the vision model; never an error. Fetch with `scripts/fetch_ocr_models.py`.
...
| [Sign Guide](documentation/SIGN_GUIDE.md) | Text repair with the Sign Tools -- the nine text classes, slop detection, glyph guidance, model choice, OCR backend installation, and troubleshooting |
